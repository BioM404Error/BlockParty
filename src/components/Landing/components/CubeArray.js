...
const CubeArray = (count, start, end, offset) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i < count / 4) {
      ...
      posY =
        Math.floor(Math.random() * (4 / 3) * 1000) / 1000 + 3.5 + 2 * offset;
    } else if (i >= (3 * count) / 4) {
      posY =
        Math.floor(Math.random() * (4 / 3) * 1000) / 1000 + 3.5 + 3 * offset;
    }
    const singleCube = (
      <LandingCube color={color} posX={posX} posY={posY} rotDir={rotDir} />
    );
    cubeArray.push(singleCube);
  }

  return <mesh>{cubeArray}</mesh>;
};
